...
def functionLike(mySystem):
    ...
    print('')
    print('\033[0;32mDelay Time Configurato a 2 sec\033[m')
    print('')
    ...
    hashtag = str(input('Digita hashtag (senza #): ')) # hashtag
    likes = int(input('Digita quanti like vuoi mettere (numero intero): ')) # amount of photos to like
    ...
    def like(likes): # function to like the photos
        ...
        while item <= likes: # loop with how many photos to like
            ...
            try:
                ...
                if driver.current_url not in likesAlready:   #check if the like has already been put
                    ...
                    sleep(random.randint(2, 10)) # break time between likes and comment due to instagram policy against bots
                    driver.find_element_by_class_name('coreSpriteRightPaginationArrow').click() # click on next photo button
                    numLike = numLike + 1
                else:
                    print('\033[0;32mLike già messo in precedenza\033[m')

                item = item + 1

            except:

                sleep(10) # if connection errors occur

        print(f'Numero di like inseriti: \033[0;33m{numLike - 1}\033[m')
